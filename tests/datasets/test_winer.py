...
def define_paths():
    ...
    return (NE_path, docs_path, vocab_path, train_processed_path, test_processed_path)
...
def test_train_test_documents(define_paths):
    ...
    assert (
        len(train_ids) == expected_train_size
        and len(test_ids) == (4 - expected_train_size)
        and len(set(train_ids + test_ids)) == 4
    )
